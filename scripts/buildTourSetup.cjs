#!/usr/bin/env node
/**
 * Erzeugt src/data/tourSetup.json:
 * - 1 Depot (N00)
 * - 18 Adressen (07–10)
 * - 50 Mid-Nodes
 * - Junctions an "lock"-Punkten
 * - 4 gesperrte Kanten (blocked)
 * - Kantenlängen in km (px / SCALE_PX_PER_KM)
 */
const fs = require("fs");
const path = require("path");

const WIDTH = 960, HEIGHT = 640, SCALE_PX_PER_KM = 110, SEED = 42;
const NUM_ADDRESSES = 18, NUM_MIDS = 50, NUM_BLOCKED = 4;
...
const rnd = lcg(SEED);
const jitter=(v,a)=>Math.round(v+(rnd()*2-1)*a);
const distPx=(a,b)=>Math.hypot(a.x-b.x,a.y-b.y);
const toKm=(px)=>+(px/ SCALE_PX_PER_KM).toFixed(3);
...
const P = pts => pts.map(p=>({...p,lock:!!p.lock}));
...
const nodes=[]; const nodeByKey=new Map();
function addNode(type,x,y,label){
  let id; if(type==="depot") id="N00";
  else if(type==="address") id=newAddrId();
  else if(type==="junction") id=newJuncId();
  else if(type==="mid") id=newMidId(); else throw new Error("type?");
  const n={id,type,x:Math.round(x),y:Math.round(y)}; if(label) n.label=label; nodes.push(n); return n;
}
function addOrGetJunction(x,y){
  const k=`${Math.round(x)}|${Math.round(y)}`;
  if(nodeByKey.has(k)) return nodeByKey.get(k);
  const n=addNode("junction",x,y); nodeByKey.set(k,n); return n;
}
...
function addExtraMids(target){
  const count=()=>nodes.filter(n=>n.type==="mid").length;
  while(count()<target){
    const li=Math.floor(rnd()*streetNodes.length), line=streetNodes[li];
    if(line.length<2) continue;
    const si=Math.floor(rnd()*(line.length-1)), a=line[si], b=line[si+1];
    const t=0.25+rnd()*0.5, x=jitter(a.x+t*(b.x-a.x),4), y=jitter(a.y+t*(b.y-a.y),4);
    const m=addNode("mid",x,y); line.splice(si+1,0,m);
  }
}
addExtraMids(50);

function shuffled(arr){ const r=arr.slice(); for(let i=r.length-1;i>0;i--){ const j=Math.floor(rnd()*(i+1)); [r[i],r[j]]=[r[j],r[i]];} return r; }
const cands=shuffled(nodes.filter(n => n.type!=="depot"));
let addr=0; for(const n of cands){ if(addr>=18) break; const a=addNode("address",n.x,n.y,`Adr ${addr+1}`); a.timeWindow={start:"07:00",end:"10:00"}; addr++; }

const edges=[]; const toKmLen=(a,b)=>toKm(distPx(a,b));
function addEdge(a,b){ edges.push({ id:`E${String(++eid).padStart(2,"0")}`, a:a.id, b:b.id, lengthKm: toKmLen(a,b)}); }
for(const line of streetNodes){ for(let i=0;i<line.length-1;i++){ addEdge(line[i], line[i+1]); } }

function markBlocked(n=4){
  const ok=edges.filter(e=>{
    const A=nodes.find(n=>n.id===e.a), B=nodes.find(n=>n.id===e.b);
    return e.lengthKm>=0.2 && !(A.type==="depot"||B.type==="depot");
  });
  const mix=shuffled(ok); mix.slice(0,n).forEach(e=>e.blocked=true);
}
markBlocked(4);

const tourSetup = {
  canvas:{width:960,height:640,scalePxPerKm:110},
  nodes, edges, baseline: BASELINE
};
